...
export class ProfileService {
  ...
  async getAccountProfile(user: UserRecord) {
    const player = await this.repository.findLobbyPlayerByClientId(accountProfileClientId(user.id));
    if (!player) {
      return failure(404, "Chua co ho so UID");
    }
    return this.buildProfile(player);
  }
  ...
  async updateAccountProfile(user: UserRecord, payload: Record<string, unknown>, mirrorClientId?: string) {
    const accountClientId = accountProfileClientId(user.id);
    const accountProfile = await this.repository.findLobbyPlayerByClientId(accountClientId);
    if (!accountProfile) {
      return failure(404, "Chua co ho so UID");
    }

    const updatedAccount = await this.applyProfileUpdates(accountProfile.id, payload);
    if (!updatedAccount.ok) return updatedAccount;

    if (mirrorClientId) {
      const current = await this.repository.findLobbyPlayerByClientId(mirrorClientId);
      if (current) {
        await this.applyProfileUpdates(current.id, payload);
      }
    }
  ...
  private async buildProfile(player: Awaited<ReturnType<BanPickRepository["findLobbyPlayerByClientId"]>>) {
    if (!player) return failure(404, "Chua dang ky UID");
  ...
  private async applyProfileUpdates(playerId: string, payload: Record<string, unknown>) {
    const updates: { displayName?: string | null; customAvatarUrl?: string | null } = {};
    ...
    const updated = await this.repository.updateLobbyPlayerById(playerId, updates);
    return success({
      profile: {
        uid: updated.uid,
        nickname: updated.nickname,
