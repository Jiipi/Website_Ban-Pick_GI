...
1. Presentation: `src/app`, `src/components`, `src/presentation`
   ...
   - Must not import Prisma, repositories, or infrastructure adapters directly.
...
2. Application: `src/application`
   ...
   - Depends on domain policies and port interfaces, not Prisma.

3. Domain: `src/domain`
   - Contains pure business rules and types.
   - No Next.js, Prisma, Supabase, fetch, cookies, or HTTP response objects.

4. Infrastructure: `src/infrastructure`
   - Implements application ports using Prisma, Supabase, Enka, and Genshin data sources.
   - May import `src/lib/prisma` and external SDKs.
...
- Never import `@/lib/prisma`, `@/infrastructure/*`, `@prisma/client`, or `@supabase/supabase-js` from `src/app` or `src/components`.
