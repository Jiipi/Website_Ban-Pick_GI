...
OUTPUT = ROOT / "BaoCao_TongHop_Website_BanPick_Genshin.docx"
...
TABLES = [
    ...
    "Bảng 3.7. Bảng màu chủ đạo (Color Palette) của giao diện",
    "Bảng 3.8. Hệ thống kiểu chữ (Typography Hierarchy)",
    "Bảng 3.9. Quy ước trạng thái nhân vật và phản hồi tương tác",
...
SECTIONS = {
    ...
    "CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG": [
        ...
        "3.18. Design System và Style Guide",
    ],
...
def chapter1(doc):
    ...
    section(doc, SECTIONS["CHƯƠNG 1: TỔNG QUAN ĐỀ TÀI"][0], [
        ...
        "Điểm đặc thù của cộng đồng người chơi Genshin Impact là mức độ phát triển tài khoản có sự khác biệt đáng kể. Nhân vật 5 sao, cung mệnh cao hoặc vũ khí 5 sao có thể tạo lợi thế rõ rệt. Vì vậy, nhiều giải đấu cộng đồng áp dụng luật Cost và Time Handicap để cân bằng trận đấu, thay vì để hai đội tự do lựa chọn đội hình mạnh nhất.",
        "Một trận La Hoàn Cảnh Giới đối kháng giữa hai người chơi thường gồm hai bước: lựa chọn đội hình và thi đấu hoàn thành thử thách. Bước thứ hai diễn ra trong môi trường trò chơi, còn bước đầu tiên thường được tổ chức bên ngoài trò chơi. Đây là nhu cầu còn thiếu mà các công cụ cộng đồng cần đáp ứng, đồng thời là cơ sở để website Ban/Pick có giá trị ứng dụng riêng.",
    ...
    section(doc, SECTIONS["CHƯƠNG 1: TỔNG QUAN ĐỀ TÀI"][3], [
        ...
        "Các mục tiêu này có quan hệ chặt chẽ. Nếu chỉ đạt mục tiêu kỹ thuật mà không có nghiệp vụ rõ ràng, hệ thống sẽ chỉ dừng lại ở mức nguyên mẫu kỹ thuật. Ngược lại, nếu chỉ đạt mục tiêu nghiệp vụ nhưng cấu trúc mã nguồn thiếu tổ chức, dự án sẽ gặp khó khăn trong quá trình mở rộng. Báo cáo vì vậy đối chiếu cả hai khía cạnh khi đánh giá mức độ hoàn thành.",
    ])
    ...
    add_extras = [
        (SECTIONS["CHƯƠNG 1: TỔNG QUAN ĐỀ TÀI"][6], [
            "Nếu chỉ xem website là nơi thực hiện thao tác lựa chọn nhân vật, phạm vi đề tài sẽ trở nên hạn chế. Tuy nhiên, khi đặt trong bối cảnh tổ chức giải đấu cộng đồng, sản phẩm phải xử lý nhiều yếu tố hơn: quyền điều phối, trạng thái phòng, lịch sử trận, dữ liệu đội hình và trải nghiệm người xem.",
        ...
        (SECTIONS["CHƯƠNG 1: TỔNG QUAN ĐỀ TÀI"][9], [
            ...
            "Cách tiếp cận trên giúp báo cáo đi từ bối cảnh thực tế, qua phân tích thiết kế, sau đó đến xây dựng và đánh giá kết quả theo trình tự học thuật rõ ràng.",
        ]),
    ]
...
def chapter2(doc):
    ...
    contents = {
        ...
        SECTIONS["CHƯƠNG 2: CÔNG NGHỆ VÀ CÔNG CỤ PHÁT TRIỂN"][7]: [
            "Trong quá trình thực hiện đề tài, các công cụ AI được sử dụng như một phương tiện hỗ trợ phân tích, gợi ý hướng tổ chức mã nguồn, rà soát lỗi diễn đạt và hệ thống hóa nội dung báo cáo. Tuy nhiên, các quyết định liên quan đến phạm vi chức năng, thiết kế cơ sở dữ liệu, lựa chọn công nghệ và đánh giá kết quả đều được đối chiếu với mã nguồn và sản phẩm đã triển khai.",
            "AI đóng vai trò hỗ trợ quá trình học tập và phát triển, không thay thế vai trò phân tích, thiết kế và hiện thực của người thực hiện đề tài. Các nội dung kỹ thuật, phạm vi chức năng và kết quả đánh giá vẫn được kiểm chứng thông qua mã nguồn, tài liệu dự án và kết quả kiểm thử.",
    ...
    extra_section(doc, SECTIONS["CHƯƠNG 2: CÔNG NGHỆ VÀ CÔNG CỤ PHÁT TRIỂN"][9], [
        "Stack công nghệ của đề tài được lựa chọn theo tiêu chí vừa đủ cho một sản phẩm có khả năng triển khai thực tế nhưng không vượt quá phạm vi học phần. Next.js gom frontend và backend trong cùng dự án; Supabase giảm thời gian dựng hạ tầng; Prisma giúp dữ liệu rõ ràng; Tailwind hỗ trợ xây dựng giao diện nhanh.",
        ...
        "Nếu chỉ dùng localStorage hoặc dữ liệu tĩnh, việc dựng nguyên mẫu có thể nhanh hơn nhưng không phản ánh đúng nhu cầu lưu lịch sử và đồng bộ realtime. Vì vậy, PostgreSQL và Supabase là lựa chọn phù hợp để dữ liệu phòng đấu có tính bền vững.",
...
def chapter3(doc):
    ...
    table(doc, TABLES[4], ["Nhóm", "Yêu cầu", "Cách đáp ứng"], [
        ...
        ["Triển khai", "Có thể vận hành ngoài máy cá nhân", "Docker, env vars, Supabase/PostgreSQL"],
    ])
    ...
    section(doc, SECTIONS["CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG"][10], [
        ...
        "Định hướng thiết kế giao diện ưu tiên bố cục MOBA ba cột trên desktop: đội Xanh ở bên trái, pool nhân vật ở giữa và đội Đỏ ở bên phải. Trên thiết bị nhỏ, bố cục cần chuyển thành một cột để người dùng vẫn theo dõi được lượt và danh sách nhân vật mà không phải thu phóng thủ công.",
    ])
    figure(doc, FIGURES[4], ["Desktop: [Đội Xanh]  [Pool nhân vật]  [Đội Đỏ]", "Mobile: Pool nhân vật", "        ↓", "        Đội Xanh", "        ↓", "        Đội Đỏ"])
    table(doc, TABLES[8], ["Vai trò màu", "Mã màu gợi ý", "Cách sử dụng trong website"], [
        ["Nền chính", "#0b0c10 / #101524", "Tạo nền tối, giảm chói khi quan sát lâu"],
        ["Primary accent", "#66FCF1", "Nút hành động chính, viền highlight lượt"],
        ["Vàng kim", "#F7C948", "Nhấn mạnh kết quả, Cost và thông tin quan trọng"],
        ["Đội Xanh", "#3B82F6", "Khu vực Blue, trạng thái đến lượt Blue"],
        ["Đội Đỏ", "#EF4444", "Khu vực Red, trạng thái đến lượt Red"],
        ["Tím 4 sao", "#A855F7", "Nền/viền nhân vật hoặc vũ khí 4 sao"],
        ["Vàng 5 sao", "#F59E0B", "Nền/viền nhân vật hoặc vũ khí 5 sao"],
    ])
    table(doc, TABLES[9], ["Cấp chữ", "Mục đích", "Quy ước hiển thị"], [
        ["H1", "Tên phòng, tiêu đề trang", "Lớn, đậm, tương phản cao"],
        ["H2", "Tên đội, phase Ban/Pick", "Vừa, đậm, đổi màu theo đội đang đến lượt"],
        ["Body", "Tên nhân vật, chat, mô tả", "Dễ đọc, không quá nhỏ trên mobile"],
        ["Metadata", "Cost, UID, trạng thái phụ", "Nhỏ hơn body nhưng đủ tương phản"],
    ])
    table(doc, TABLES[10], ["Trạng thái", "Biểu hiện giao diện", "Ý nghĩa UX"], [
        ["Nhân vật khả dụng", "Avatar rõ, hover nổi nhẹ", "Có thể chọn khi đúng lượt"],
        ["Đã bị ban", "Grayscale, opacity thấp, dấu gạch chéo", "Không thể chọn lại"],
        ["Đã được pick", "Khóa click, hiển thị ở đội tương ứng", "Nhân vật đã thuộc đội"],
        ["Đang đến lượt", "Viền sáng/pulse ở khu vực đội", "Giúp player nhận biết cần thao tác"],
        ["Đang lưu build", "Thông báo nhỏ hoặc trạng thái pending", "Cho biết dữ liệu đang đồng bộ"],
    ])
    section(doc, SECTIONS["CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG"][17], [
        "Design System của website được xây dựng để giữ tính nhất quán giữa các màn hình như trang chủ, lobby, phòng draft, build phase, result và tournament. Bảng màu tối kết hợp xanh/đỏ theo đội giúp giao diện có bản sắc gaming, đồng thời vẫn hỗ trợ người xem phân biệt hai bên nhanh chóng.",
        "Hệ thống kiểu chữ ưu tiên khả năng đọc trong bối cảnh thi đấu. Tên phòng, phase và lượt hiện tại cần nổi bật hơn tên nhân vật hoặc thông tin phụ. Cách phân cấp này giúp trọng tài và player xác định hành động chính mà không phải đọc toàn bộ giao diện.",
        "Các micro-interaction như hiệu ứng hover, viền sáng nhịp nhàng cho lượt hiện tại, hiệu ứng mờ nhân vật đã ban và khóa thao tác cho nhân vật đã pick vừa tạo cảm giác liền mạch, vừa đóng vai trò phản hồi trạng thái. Người dùng có thể nhận biết nhanh chóng vì sao một nhân vật không thể được chọn và đội nào đang có quyền thao tác.",
    ...
    chapter3_extras = [
        ...
        (SECTIONS["CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG"][12], [
            ...
            "Cách phân loại này còn hỗ trợ định hướng phát triển sau đề tài. Nếu có thêm thời gian, ưu tiên đầu tiên là hoàn thiện chức năng cốt lõi; các chức năng mở rộng sẽ được phát triển dần dần khi nhu cầu cộng đồng được xác định rõ hơn.",
        ]),
        ...
        (SECTIONS["CHƯƠNG 3: PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG"][16], [
            ...
            "Thiết kế cho khán giả cũng ảnh hưởng đến cách đặt màu và kích thước chữ. Những con số như tổng Cost, thời gian handicap hoặc lượt hiện tại phải đủ nổi bật để người xem có thể nhận biết nhanh chóng dù không đọc toàn bộ thông tin trên giao diện.",
            "Thành phần này cho thấy website có khả năng phát triển theo hướng eSports cộng đồng. Sản phẩm không chỉ phục vụ người dùng trực tiếp thao tác, mà còn hỗ trợ cách trận đấu được trình bày trước người xem.",
        ]),
    ]
...
def chapter4(doc):
    ...
    for title, paras in [
        ...
        (SECTIONS["CHƯƠNG 4: XÂY DỰNG VÀ TRIỂN KHAI HỆ THỐNG"][4], [
            "Ngoài draft và result, website còn có các chức năng giúp trận đấu vận hành liền mạch hơn. Timer và bank time tạo áp lực thời gian; pause/undo hỗ trợ xử lý sự cố; chat giúp host và player trao đổi ngay trong phòng.",
        ...
        (SECTIONS["CHƯƠNG 4: XÂY DỰNG VÀ TRIỂN KHAI HỆ THỐNG"][5], [
            ...
            "Việc có nền tảng mở rộng cho thấy sản phẩm không chỉ là bản minh họa một màn hình. Tuy nhiên, báo cáo vẫn đặt chức năng lõi Ban/Pick, build và result làm trọng tâm đánh giá.",
        ...
        section(doc, title, paras)
    table(doc, TABLES[11], ["Tầng", "Thư mục/file tiêu biểu", "Vai trò"], [
    ...
    table(doc, TABLES[12], ["Mã", "Kịch bản", "Dữ liệu thử", "Mong đợi", "Thực tế", "Đánh giá"], [
    ...
    for caption, lines in [
        (FIGURES[5], ["Trang chủ giới thiệu mục đích website", "Nút tạo phòng/tham gia", "Điều hướng đến lobby, tournament, công cụ"]),
        (FIGURES[6], ["Hai khu vực đội Xanh/Đỏ", "Lượt hiện tại và timer", "Danh sách nhân vật với trạng thái ban/pick"]),
        (FIGURES[7], ["Form nhập rarity, cung mệnh, vũ khí", "Tính Cost theo từng nhân vật", "Dữ liệu chuẩn bị cho trang kết quả"]),
        (FIGURES[8], ["Tổng Cost hai đội", "Chênh lệch và Time Handicap", "Danh sách đội hình đã chọn"]),
        (FIGURES[9], ["Danh sách giải đấu", "Participant và bracket", "Liên kết match với phòng đấu"]),
    ]:
    ...
    chapter4_extras = [
        ...
        (SECTIONS["CHƯƠNG 4: XÂY DỰNG VÀ TRIỂN KHAI HỆ THỐNG"][14], [
            ...
            "Triển khai không kết thúc ở bước container có thể vận hành. Sản phẩm cần được kiểm tra lại trên môi trường thật: tạo phòng, join, đồng bộ realtime, đọc/ghi database và truy cập dữ liệu ngoài. Đây là bước quan trọng trước khi đưa hệ thống vào sử dụng cho một buổi thi đấu thực tế.",
        ]),
    ]
...
def chapter5(doc):
    ...
    table(doc, TABLES[13], ["Mục tiêu ban đầu", "Kết quả hiện tại", "Mức độ"], [
    ...
    for title, paras in [
        ...
        (SECTIONS["CHƯƠNG 5: TỔNG KẾT, ĐÁNH GIÁ VÀ HƯỚNG PHÁT TRIỂN"][5], [
            ...
            "Tổng kết lại, đề tài đã xây dựng được nền tảng website Ban/Pick Genshin Impact có tính thực tiễn, có quy trình rõ ràng và có khả năng phát triển tiếp. Dù vẫn còn hạn chế, sản phẩm đã thể hiện được cách ứng dụng công nghệ web hiện đại vào một nhu cầu cụ thể của cộng đồng người chơi trò chơi Genshin Impact.",
        ]),
    ]:
        section(doc, title, paras)
    ...
    extra_section(doc, SECTIONS["CHƯƠNG 5: TỔNG KẾT, ĐÁNH GIÁ VÀ HƯỚNG PHÁT TRIỂN"][7], [
        ...
        "Cách trình bày này giúp báo cáo có tính kỹ thuật nhưng vẫn giữ được tinh thần tổng kết sản phẩm, hướng tới người đọc cần đánh giá tổng quan thay vì chỉ liệt kê thư viện và thư mục.",
    ])
    extra_section(doc, SECTIONS["CHƯƠNG 5: TỔNG KẾT, ĐÁNH GIÁ VÀ HƯỚNG PHÁT TRIỂN"][8], [
        ...
        "Trong quá trình dùng thử, cần ghi nhận những điểm người chơi phải hỏi lại, thao tác nhầm hoặc chờ quá lâu. Những phản hồi này có giá trị hơn cảm nhận chủ quan của người phát triển vì chúng phản ánh trải nghiệm sử dụng thực tế.",
